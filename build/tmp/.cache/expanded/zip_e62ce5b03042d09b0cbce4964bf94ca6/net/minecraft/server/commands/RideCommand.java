...
public class RideCommand {
   private static final DynamicCommandExceptionType ERROR_NOT_RIDING = new DynamicCommandExceptionType(
      p_265076_ -> Component.translatable("commands.ride.not_riding", p_265076_)
   );
   private static final Dynamic2CommandExceptionType ERROR_ALREADY_RIDING = new Dynamic2CommandExceptionType(
      (p_265488_, p_265072_) -> Component.translatable("commands.ride.already_riding", p_265488_, p_265072_)
   );
   private static final Dynamic2CommandExceptionType ERROR_MOUNT_FAILED = new Dynamic2CommandExceptionType(
      (p_265321_, p_265603_) -> Component.translatable("commands.ride.mount.failure.generic", p_265321_, p_265603_)
   );
                     ...
                     Commands.literal("mount")
                        .then(
                           Commands.argument("vehicle", EntityArgument.entity())
                              .executes(
                                 p_265139_ -> mount(
                                       p_265139_.getSource(), EntityArgument.getEntity(p_265139_, "target"), EntityArgument.getEntity(p_265139_, "vehicle")
                                    )
                              )
                        )
                  )
                  .then(Commands.literal("dismount").executes(p_265418_ -> dismount(p_265418_.getSource(), EntityArgument.getEntity(p_265418_, "target"))))
            )
      );
   }
