...
public class PiglinAi {
   ...
   public static void angerNearbyPiglins(Player p_34874_, boolean p_34875_) {
      ...
      list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !p_34875_ || BehaviorUtils.canSee(p_34881_, p_34874_)).forEach(p_301905_ -> {
         if (p_301905_.level().getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
            setAngerTargetToNearestTargetablePlayerIfFound(p_301905_, p_34874_);
         } else {
            setAngerTarget(p_301905_, p_34874_);
         }
      });
   }
   ...
   protected static void wasHurtBy(Piglin p_34838_, LivingEntity p_34839_) {
      if (!(p_34839_ instanceof Piglin)) {
         ...
         getAvoidTarget(p_34838_).ifPresent(p_302207_ -> {
            if (p_302207_.getType() != p_34839_.getType()) {
   ...
   private static BehaviorControl<LivingEntity> babySometimesRideBabyHoglin() {
      ...
      return CopyMemoryWithExpiry.create(
         p_301900_ -> p_301900_.isBaby() && setentitylooktargetsometimes$ticker.tickDownAndCheck(p_301900_.level().random),
   ...
   protected static void broadcastAngerTarget(AbstractPiglin p_34896_, LivingEntity p_34897_) {
      getAdultPiglins(p_34896_).forEach(p_302204_ -> {
         if (p_34897_.getType() != EntityType.HOGLIN || p_302204_.canHunt() && ((Hoglin)p_34897_).canBeHunted()) {
            setAngerTargetIfCloserThanCurrent(p_302204_, p_34897_);
         }
      });
   }
