...
public class ChunkStatus {
   ...
   public static final ChunkStatus STRUCTURE_STARTS = register(
      ...
      (p_301963_, p_301964_, p_301965_, p_301966_, p_301967_, p_301968_, p_301969_, p_301970_, p_301971_) -> {
         if (p_301965_.getServer().getWorldData().worldGenOptions().generateStructures()) {
            p_301966_.createStructures(
               p_301965_.registryAccess(), p_301965_.getChunkSource().getGeneratorState(), p_301965_.structureManager(), p_301971_, p_301967_
            );
         }
   
         p_301965_.onStructureStartsAvailable(p_301971_);
         return CompletableFuture.completedFuture(Either.left(p_301971_));
      },
   ...
   public static final ChunkStatus STRUCTURE_REFERENCES = registerSimple(
      "structure_references", STRUCTURE_STARTS, 8, PRE_FEATURES, ChunkStatus.ChunkType.PROTOCHUNK, (p_196843_, p_196844_, p_196845_, p_196846_, p_196847_) -> {
   ...
   public static final ChunkStatus CARVERS = registerSimple(
      ...
      POST_FEATURES,
      ChunkStatus.ChunkType.PROTOCHUNK,
      (p_301958_, p_301959_, p_301960_, p_301961_, p_301962_) -> {
         WorldGenRegion worldgenregion = new WorldGenRegion(p_301959_, p_301961_, p_301958_, 0);
         if (p_301962_ instanceof ProtoChunk protochunk) {
            Blender.addAroundOldChunksCarvingMaskFilter(worldgenregion, protochunk);
         }
   
         p_301960_.applyCarvers(
            worldgenregion,
            p_301959_.getSeed(),
            p_301959_.getChunkSource().randomState(),
            p_301959_.getBiomeManager(),
            p_301959_.structureManager().forWorldGenRegion(worldgenregion),
            p_301962_,
   ...
   private static CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> initializeLight(
      ThreadedLevelLightEngine p_282288_, ChunkAccess p_282906_
   ) {
   ...
   private static ChunkStatus registerSimple(
      String p_62415_,
      @Nullable ChunkStatus p_62416_,
      int p_62417_,
      EnumSet<Heightmap.Types> p_62418_,
      ChunkStatus.ChunkType p_62419_,
   ...
   private static ChunkStatus register(
      String p_62400_,
      @Nullable ChunkStatus p_62401_,
      int p_62402_,
      EnumSet<Heightmap.Types> p_62403_,
      ChunkStatus.ChunkType p_62404_,
   ...
   private static ChunkStatus register(
      String p_282817_,
      @Nullable ChunkStatus p_282644_,
      int p_281535_,
      boolean p_282329_,
      EnumSet<Heightmap.Types> p_281310_,
      ChunkStatus.ChunkType p_281968_,
      ChunkStatus.GenerationTask p_283654_,
      ChunkStatus.LoadingTask p_282175_
   ) {
      return Registry.register(
         BuiltInRegistries.CHUNK_STATUS, p_282817_, new ChunkStatus(p_282644_, p_281535_, p_282329_, p_281310_, p_281968_, p_283654_, p_282175_)
      );
   }
   ...
   public ChunkStatus(
      @Nullable ChunkStatus p_289640_,
      int p_289655_,
      boolean p_289657_,
      EnumSet<Heightmap.Types> p_289662_,
      ChunkStatus.ChunkType p_289652_,
      ChunkStatus.GenerationTask p_289679_,
      ChunkStatus.LoadingTask p_289646_
   ) {
   ...
   public CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> generate(
      Executor p_283276_,
      ServerLevel p_281420_,
      ChunkGenerator p_281836_,
      StructureTemplateManager p_281305_,
      ThreadedLevelLightEngine p_282570_,
      Function<ChunkAccess, CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>>> p_283114_,
      List<ChunkAccess> p_282723_
   ) {
      ...
      return this.generationTask
         .doWork(this, p_283276_, p_281420_, p_281836_, p_281305_, p_282570_, p_283114_, p_282723_, chunkaccess)
         .thenApply(
   ...
   interface SimpleGenerationTask extends ChunkStatus.GenerationTask {
      ...
      default CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> doWork(
         ChunkStatus p_281382_,
         Executor p_283285_,
         ServerLevel p_283408_,
         ChunkGenerator p_282263_,
         StructureTemplateManager p_282374_,
         ThreadedLevelLightEngine p_281701_,
         Function<ChunkAccess, CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>>> p_282473_,
         List<ChunkAccess> p_282316_,
         ChunkAccess p_281861_
      ) {
