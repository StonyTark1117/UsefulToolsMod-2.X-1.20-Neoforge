...
public class WorldOpenFlows {
   ...
   public void createLevelFromExistingSettings(
      ...
      CloseableResourceManager closeableresourcemanager = new WorldLoader.PackConfig(packrepository, p_251654_.getDataConfiguration(), false, false)
         ...
         .getSecond();
      this.minecraft
         .doWorldLoad(p_250919_.getLevelId(), p_250919_, packrepository, new WorldStem(closeableresourcemanager, p_248897_, p_250801_, p_251654_), true);
   }
   ...
   private WorldStem loadWorldStem(LevelStorageSource.LevelStorageAccess p_233123_, boolean p_233124_, PackRepository p_233125_) throws Exception {
      ...
      return this.loadWorldDataBlocking(
         ...
         p_247851_ -> {
            ...
            Pair<WorldData, WorldDimensions.Complete> pair = p_233123_.getDataTag(
               dynamicops, p_247851_.dataConfiguration(), registry, p_247851_.datapackWorldgen().allRegistriesLifecycle()
            );
            if (pair == null) {
   ...
   public Pair<LevelSettings, WorldCreationContext> recreateWorldData(LevelStorageSource.LevelStorageAccess p_249540_) throws Exception {
      ...
      return this.<Data, Pair<LevelSettings, WorldCreationContext>>loadWorldDataBlocking(
         ...
         p_247857_ -> {
            ...
            Pair<WorldData, WorldDimensions.Complete> pair = p_249540_.getDataTag(
               dynamicops, p_247857_.dataConfiguration(), registry, p_247857_.datapackWorldgen().allRegistriesLifecycle()
            );
            if (pair == null) {
               throw new IllegalStateException("Failed to load world");
            } else {
               return new WorldLoader.DataLoadOutput<>(
                  new Data(pair.getFirst().getLevelSettings(), pair.getFirst().worldGenOptions(), pair.getSecond().dimensions()),
                  p_247857_.datapackDimensions()
               );
            }
         },
   ...
   private void doLoadLevel(Screen p_233146_, String p_233147_, boolean p_233148_, boolean p_233149_, boolean confirmExperimentalWarning) {
      ...
      if (levelstoragesource$levelstorageaccess != null) {
         ...
         try {
            ...
            if (!p_233148_) {
               this.minecraft.setScreen(new DatapackLoadFailureScreen(() -> this.doLoadLevel(p_233146_, p_233147_, true, p_233149_)));
            } else {
               this.minecraft
                  .setScreen(
                     new AlertScreen(
         ...
         if (skipConfirmation || !p_233149_ || !flag && !flag1) {
            this.minecraft
               ...
               .loadBundledResourcePack(levelstoragesource$levelstorageaccess)
               .thenApply(p_233177_ -> true)
               .exceptionallyComposeAsync(p_233183_ -> {
                  LOGGER.warn("Failed to load pack: ", p_233183_);
                  return this.promptBundledPackLoadFailure();
               }, this.minecraft)
               .thenAcceptAsync(p_233168_ -> {
                  if (p_233168_) {
                     this.minecraft.doWorldLoad(p_233147_, levelstoragesource$levelstorageaccess, packrepository, worldstem, false);
                  } else {
                     worldstem.close();
                     safeCloseAccess(levelstoragesource$levelstorageaccess, p_233147_);
                     this.minecraft.getDownloadedPackSource().clearServerPack().thenRunAsync(() -> this.minecraft.setScreen(p_233146_), this.minecraft);
                  }
               }, this.minecraft)
               .exceptionally(p_233175_ -> {
            ...
            this.askForBackup(p_233146_, p_233147_, flag, () -> this.doLoadLevel(p_233146_, p_233147_, p_233148_, false));
            else net.neoforged.neoforge.client.ClientHooks.createWorldConfirmationScreen(() -> this.doLoadLevel(p_233146_, p_233147_, p_233148_, false, true));
            worldstem.close();
            safeCloseAccess(levelstoragesource$levelstorageaccess, p_233147_);
         }
      }
