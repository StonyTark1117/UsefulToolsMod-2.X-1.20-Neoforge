   ...
   public void setupCollections(Iterable<RecipeHolder<?>> p_266814_, RegistryAccess p_266878_) {
      ...
      map.forEach(
         (p_266602_, p_266603_) -> map1.put(
               p_266602_,
               p_266603_.stream().map(p_266605_ -> new RecipeCollection(p_266878_, p_266605_)).peek(builder::add).collect(ImmutableList.toImmutableList())
            )
      );
      RecipeBookCategories.AGGREGATE_CATEGORIES
         .forEach(
            (p_90637_, p_90638_) -> map1.put(
                  p_90637_,
                  p_90638_.stream().flatMap(p_167706_ -> map1.getOrDefault(p_167706_, ImmutableList.of()).stream()).collect(ImmutableList.toImmutableList())
               )
         );
   ...
   private static RecipeBookCategories getCategory(RecipeHolder<?> p_301136_) {
      ...
      if (recipe instanceof CraftingRecipe craftingrecipe) {
         ...
         if (recipetype == RecipeType.STONECUTTING) {
            ...
            LOGGER.warn(
               "Unknown recipe category: {}/{}", LogUtils.defer(() -> BuiltInRegistries.RECIPE_TYPE.getKey(recipe.getType())), LogUtils.defer(p_301136_::id)
            );
