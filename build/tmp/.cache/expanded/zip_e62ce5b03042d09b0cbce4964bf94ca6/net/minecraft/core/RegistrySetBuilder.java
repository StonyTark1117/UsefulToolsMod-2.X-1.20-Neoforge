...
public class RegistrySetBuilder {
   ...
   public HolderLookup.Provider build(RegistryAccess p_256112_) {
      ...
      Stream<HolderLookup.RegistryLookup<?>> stream1 = this.entries
         .stream()
         .map(p_255700_ -> p_255700_.collectChanges(registrysetbuilder$buildstate).buildAsLookup());
   ...
   public HolderLookup.Provider buildPatch(RegistryAccess p_255676_, HolderLookup.Provider p_255900_) {
      ...
      HolderLookup.Provider holderlookup$provider = HolderLookup.Provider.create(
         Stream.concat(stream, map.values().stream().map(RegistrySetBuilder.RegistryContents::buildAsLookup).peek(registrysetbuilder$buildstate::addOwner))
      );
   ...
   static record BuildState(
      ...
      public void fillMissingHolders(HolderLookup.Provider p_255679_) {
         ...
         while(iterator.hasNext()) {
            Entry<ResourceKey<Object>, Holder.Reference<Object>> entry = iterator.next();
            ResourceKey<Object> resourcekey = entry.getKey();
            Holder.Reference<Object> reference = entry.getValue();
            map.computeIfAbsent(resourcekey.registry(), p_255896_ -> p_255679_.lookup(ResourceKey.createRegistryKey(p_255896_)))
               .flatMap(p_256068_ -> p_256068_.get(resourcekey))
               .ifPresent(p_256030_ -> {
      ...
      public Stream<RegistrySetBuilder.RegistryContents<?>> collectReferencedRegistries() {
         return this.lookup
            ...
            .distinct()
            .map(p_272342_ -> new RegistrySetBuilder.RegistryContents(ResourceKey.createRegistryKey(p_272342_), Lifecycle.stable(), Map.of()));
      }
   }
            ...
            private final Map<ResourceKey<T>, Holder.Reference<T>> entries = RegistryContents.this.values
               ...
               .collect(
                  Collectors.toUnmodifiableMap(
                     Entry::getKey,
                     p_256193_ -> {
                        RegistrySetBuilder.ValueAndHolder<T> valueandholder = (RegistrySetBuilder.ValueAndHolder)p_256193_.getValue();
                        Holder.Reference<T> reference = valueandholder.holder()
                           .orElseGet(() -> Holder.Reference.createStandAlone(this, (ResourceKey<T>)p_256193_.getKey()));
   ...
   static record RegistryStub<T>(ResourceKey<? extends Registry<T>> key, Lifecycle lifecycle, RegistrySetBuilder.RegistryBootstrap<T> bootstrap) {
      ...
      public RegistrySetBuilder.RegistryContents<T> collectChanges(RegistrySetBuilder.BuildState p_256416_) {
         ...
         Iterator<Entry<ResourceKey<?>, RegistrySetBuilder.RegisteredValue<?>>> iterator = p_256416_.registeredValues.entrySet().iterator();
