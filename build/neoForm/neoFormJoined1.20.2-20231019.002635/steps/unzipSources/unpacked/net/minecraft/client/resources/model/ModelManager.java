...
public class ModelManager implements PreparableReloadListener, AutoCloseable {
   ...
   public final CompletableFuture<Void> reload(
      ...
      return CompletableFuture.allOf(Stream.concat(map.values().stream(), Stream.of(completablefuture2)).toArray(p_250157_ -> new CompletableFuture[p_250157_]))
         .thenApplyAsync(
      ...
      return CompletableFuture.<Map<ResourceLocation, List<Resource>>>supplyAsync(
         ...
         .thenCompose(
            p_250744_ -> {
               ...
               return Util.sequence(list)
                  .thenApply(p_248966_ -> p_248966_.stream().filter(Objects::nonNull).collect(Collectors.toUnmodifiableMap(Pair::getFirst, Pair::getSecond)));
            }
         );
   }
