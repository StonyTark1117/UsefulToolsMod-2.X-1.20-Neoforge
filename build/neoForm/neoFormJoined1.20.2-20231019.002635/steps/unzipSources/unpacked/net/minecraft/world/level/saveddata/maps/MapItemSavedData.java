...
public class MapItemSavedData extends SavedData {
   ...
   public MapItemSavedData scaled(int p_164788_) {
      return createFresh(
         (double)this.centerX,
         (double)this.centerZ,
         (byte)Mth.clamp(this.scale + p_164788_, 0, 4),
   ...
   public void tickCarriedBy(Player p_77919_, ItemStack p_77920_) {
      ...
      for(int i = 0; i < this.carriedBy.size(); ++i) {
         ...
         if (!mapitemsaveddata$holdingplayer1.player.isRemoved()
            && (mapitemsaveddata$holdingplayer1.player.getInventory().contains(p_77920_) || p_77920_.isFramed())) {
   ...
   public boolean toggleBanner(LevelAccessor p_77935_, BlockPos p_77936_) {
      ...
      if (d2 >= -63.0 && d3 >= -63.0 && d2 <= 63.0 && d3 <= 63.0) {
         ...
         if (!this.isTrackedCountOverLimit(256)) {
            ...
            this.addDecoration(mapbanner.getDecoration(), p_77935_, mapbanner.getId(), d0, d1, 180.0, mapbanner.getName());
            return true;
         }
      }
   ...
   public class HoldingPlayer {
      ...
      Packet<?> nextUpdatePacket(int p_164816_) {
         ...
         return collection == null && mapitemsaveddata$mappatch == null
            ? null
            : new ClientboundMapItemDataPacket(p_164816_, MapItemSavedData.this.scale, MapItemSavedData.this.locked, collection, mapitemsaveddata$mappatch);
      }
