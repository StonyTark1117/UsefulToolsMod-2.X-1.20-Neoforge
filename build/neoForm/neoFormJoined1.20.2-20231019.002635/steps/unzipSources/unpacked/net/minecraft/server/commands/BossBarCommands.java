...
public class BossBarCommands {
   private static final DynamicCommandExceptionType ERROR_ALREADY_EXISTS = new DynamicCommandExceptionType(
      p_136636_ -> Component.translatable("commands.bossbar.create.failed", p_136636_)
   );
   private static final DynamicCommandExceptionType ERROR_DOESNT_EXIST = new DynamicCommandExceptionType(
      p_136623_ -> Component.translatable("commands.bossbar.unknown", p_136623_)
   );
                        ...
                        .then(
                           Commands.literal("name")
                              .then(
                                 Commands.argument("name", ComponentArgument.textComponent())
                                    .executes(
                                       p_136687_ -> setName(p_136687_.getSource(), getBossBar(p_136687_), ComponentArgument.getComponent(p_136687_, "name"))
                                    )
                              )
                        )
                        .then(
                           ...
                           Commands.literal("value")
                              .then(
                                 Commands.argument("value", IntegerArgumentType.integer(0))
                                    .executes(
                                       p_136661_ -> setValue(p_136661_.getSource(), getBossBar(p_136661_), IntegerArgumentType.getInteger(p_136661_, "value"))
                                    )
                              )
                        )
                        .then(
                           Commands.literal("max")
                              .then(
                                 Commands.argument("max", IntegerArgumentType.integer(1))
                                    .executes(
                                       p_136659_ -> setMax(p_136659_.getSource(), getBossBar(p_136659_), IntegerArgumentType.getInteger(p_136659_, "max"))
                                    )
                              )
                        )
                        .then(
                           Commands.literal("visible")
                              .then(
                                 Commands.argument("visible", BoolArgumentType.bool())
                                    .executes(
                                       p_136657_ -> setVisible(p_136657_.getSource(), getBossBar(p_136657_), BoolArgumentType.getBool(p_136657_, "visible"))
                                    )
                              )
                        )
                        .then(
