...
public class ApplyBonusCount extends LootItemConditionalFunction {
   ...
   static final Codec<ApplyBonusCount.FormulaType> FORMULA_TYPE_CODEC = ResourceLocation.CODEC
      .comapFlatMap(
         p_298052_ -> {
   ...
   private static final MapCodec<ApplyBonusCount.Formula> FORMULA_CODEC = new MapCodec<ApplyBonusCount.Formula>() {
      ...
      @Override
      public <T> Stream<T> keys(DynamicOps<T> p_298639_) {
         return Stream.of(p_298639_.createString("formula"), p_298639_.createString("parameters"));
      }

      @Override
      public <T> DataResult<ApplyBonusCount.Formula> decode(DynamicOps<T> p_298305_, MapLike<T> p_298680_) {
         T t = p_298680_.get("formula");
         return t == null
            ? DataResult.error(() -> "Missing type for formula in: " + p_298680_)
            : ApplyBonusCount.FORMULA_TYPE_CODEC.decode(p_298305_, t).flatMap(p_299291_ -> {
               T t1 = Objects.requireNonNullElseGet(p_298680_.get("parameters"), p_298305_::emptyMap);
               return p_299291_.getFirst().codec().decode(p_298305_, t1).map(Pair::getFirst);
            });
      }

      public <T> RecordBuilder<T> encode(ApplyBonusCount.Formula p_298557_, DynamicOps<T> p_298595_, RecordBuilder<T> p_298462_) {
         ApplyBonusCount.FormulaType applybonuscount$formulatype = p_298557_.getType();
         p_298462_.add("formula", ApplyBonusCount.FORMULA_TYPE_CODEC.encodeStart(p_298595_, applybonuscount$formulatype));
         DataResult<T> dataresult = this.encode(applybonuscount$formulatype.codec(), p_298557_, p_298595_);
         if (dataresult.result().isEmpty() || !Objects.equals(dataresult.result().get(), p_298595_.emptyMap())) {
            p_298462_.add("parameters", dataresult);
         }

         return p_298462_;
      }

      private <T, F extends ApplyBonusCount.Formula> DataResult<T> encode(Codec<F> p_298677_, ApplyBonusCount.Formula p_298376_, DynamicOps<T> p_298812_) {
         return p_298677_.encodeStart(p_298812_, (F)p_298376_);
      }
   };
