...
public class LocateCommand {
   ...
   private static final DynamicCommandExceptionType ERROR_STRUCTURE_NOT_FOUND = new DynamicCommandExceptionType(
      p_201831_ -> Component.translatable("commands.locate.structure.not_found", p_201831_)
   );
   private static final DynamicCommandExceptionType ERROR_STRUCTURE_INVALID = new DynamicCommandExceptionType(
      p_207534_ -> Component.translatable("commands.locate.structure.invalid", p_207534_)
   );
   private static final DynamicCommandExceptionType ERROR_BIOME_NOT_FOUND = new DynamicCommandExceptionType(
      p_214514_ -> Component.translatable("commands.locate.biome.not_found", p_214514_)
   );
   private static final DynamicCommandExceptionType ERROR_POI_NOT_FOUND = new DynamicCommandExceptionType(
      p_214512_ -> Component.translatable("commands.locate.poi.not_found", p_214512_)
   );
               ...
               Commands.literal("biome")
                  .then(
                     Commands.argument("biome", ResourceOrTagArgument.resourceOrTag(p_248936_, Registries.BIOME))
                        .executes(p_258232_ -> locateBiome(p_258232_.getSource(), ResourceOrTagArgument.getResourceOrTag(p_258232_, "biome", Registries.BIOME)))
                  )
            )
            .then(
   ...
   private static Optional<? extends HolderSet.ListBacked<Structure>> getHolders(
      ...
      return p_251212_.unwrap().map(p_258231_ -> p_249691_.getHolder(p_258231_).map(p_214491_ -> HolderSet.direct(p_214491_)), p_249691_::getTag);
   }
   ...
   private static int locateStructure(CommandSourceStack p_214472_, ResourceOrTagKeyArgument.Result<Structure> p_249893_) throws CommandSyntaxException {
      ...
      HolderSet<Structure> holderset = getHolders(p_249893_, registry).orElseThrow(() -> ERROR_STRUCTURE_INVALID.create(p_249893_.asPrintable()));
   ...
   private static String getElementName(Pair<BlockPos, ? extends Holder<?>> p_249526_) {
      return p_249526_.getSecond().unwrapKey().map(p_214498_ -> p_214498_.location().toString()).orElse("[unregistered]");
   }
   ...
   public static int showLocateResult(
      ...
      String s = p_262956_.unwrap().map(p_248147_ -> p_262956_.asPrintable(), p_248143_ -> p_262956_.asPrintable() + " (" + getElementName(p_263074_) + ")");
   ...
   public static int showLocateResult(
      ...
      String s = p_263031_.unwrap()
         .map(p_214463_ -> p_214463_.location().toString(), p_248145_ -> "#" + p_248145_.location() + " (" + getElementName(p_262959_) + ")");
