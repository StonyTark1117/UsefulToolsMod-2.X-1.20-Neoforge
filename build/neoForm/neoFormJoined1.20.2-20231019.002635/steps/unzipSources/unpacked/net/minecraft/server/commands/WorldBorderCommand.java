                  ...
                  .then(
                     Commands.argument("distance", DoubleArgumentType.doubleArg(-5.999997E7F, 5.999997E7F))
                        .executes(
                           p_301735_ -> setSize(
                                 p_301735_.getSource(),
                                 p_301735_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_301735_, "distance"),
                                 0L
                              )
                        )
                        .then(
                           Commands.argument("time", IntegerArgumentType.integer(0))
                              .executes(
                                 p_301734_ -> setSize(
                                       p_301734_.getSource(),
                                       p_301734_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_301734_, "distance"),
                                       p_301734_.getSource().getLevel().getWorldBorder().getLerpRemainingTime()
                                          + (long)IntegerArgumentType.getInteger(p_301734_, "time") * 1000L
                                    )
                              )
                        )
                  )
         ...
         if (p_139255_ > 0L) {
            ...
            if (p_139254_ > d0) {
               p_139253_.sendSuccess(
                  () -> Component.translatable("commands.worldborder.set.grow", String.format(Locale.ROOT, "%.1f", p_139254_), Long.toString(p_139255_ / 1000L)),
                  true
               );
            } else {
               p_139253_.sendSuccess(
