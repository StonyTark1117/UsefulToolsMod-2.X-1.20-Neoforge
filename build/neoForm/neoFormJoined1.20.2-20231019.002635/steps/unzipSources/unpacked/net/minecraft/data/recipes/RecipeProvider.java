...
public abstract class RecipeProvider implements DataProvider {
   ...
   protected CompletableFuture<?> buildAdvancement(CachedOutput p_253674_, AdvancementHolder p_301116_) {
      return DataProvider.saveStable(p_253674_, p_301116_.value().serializeToJson(), this.advancementPathProvider.json(p_301116_.id()));
   }
   ...
   protected static void nineBlockStorageRecipes(
      ...
      nineBlockStorageRecipes(p_301057_, p_251203_, p_251689_, p_251376_, p_248771_, getSimpleRecipeName(p_248771_), null, getSimpleRecipeName(p_251689_), null);
   }
   ...
   protected static void cookRecipes(RecipeOutput p_301054_, String p_126008_, RecipeSerializer<? extends AbstractCookingRecipe> p_250529_, int p_126010_) {
      simpleCookingRecipe(p_301054_, p_126008_, p_250529_, p_126010_, Items.BEEF, Items.COOKED_BEEF, 0.35F);
      simpleCookingRecipe(p_301054_, p_126008_, p_250529_, p_126010_, Items.CHICKEN, Items.COOKED_CHICKEN, 0.35F);
      simpleCookingRecipe(p_301054_, p_126008_, p_250529_, p_126010_, Items.COD, Items.COOKED_COD, 0.35F);
      simpleCookingRecipe(p_301054_, p_126008_, p_250529_, p_126010_, Items.KELP, Items.DRIED_KELP, 0.1F);
      simpleCookingRecipe(p_301054_, p_126008_, p_250529_, p_126010_, Items.SALMON, Items.COOKED_SALMON, 0.35F);
      simpleCookingRecipe(p_301054_, p_126008_, p_250529_, p_126010_, Items.MUTTON, Items.COOKED_MUTTON, 0.35F);
      simpleCookingRecipe(p_301054_, p_126008_, p_250529_, p_126010_, Items.PORKCHOP, Items.COOKED_PORKCHOP, 0.35F);
      simpleCookingRecipe(p_301054_, p_126008_, p_250529_, p_126010_, Items.POTATO, Items.BAKED_POTATO, 0.35F);
      simpleCookingRecipe(p_301054_, p_126008_, p_250529_, p_126010_, Items.RABBIT, Items.COOKED_RABBIT, 0.35F);
   }

   protected static void simpleCookingRecipe(
      RecipeOutput p_300995_,
      String p_249709_,
      RecipeSerializer<? extends AbstractCookingRecipe> p_251876_,
      int p_249258_,
      ItemLike p_250669_,
      ItemLike p_250224_,
      float p_252138_
   ) {
      SimpleCookingRecipeBuilder.generic(Ingredient.of(p_250669_), RecipeCategory.FOOD, p_250224_, p_252138_, p_249258_, p_251876_)
         .unlockedBy(getHasName(p_250669_), has(p_250669_))
         .save(p_300995_, getItemName(p_250224_) + "_from_" + p_249709_);
   }

   protected static void waxRecipes(RecipeOutput p_301254_) {
      HoneycombItem.WAXABLES
         .get()
         .forEach(
            (p_300710_, p_300711_) -> ShapelessRecipeBuilder.shapeless(RecipeCategory.BUILDING_BLOCKS, p_300711_)
                  .requires(p_300710_)
                  .requires(Items.HONEYCOMB)
                  .group(getItemName(p_300711_))
                  .unlockedBy(getHasName(p_300710_), has(p_300710_))
                  .save(p_301254_, getConversionRecipeName(p_300711_, Items.HONEYCOMB))
         );
   }

   protected static void generateRecipes(RecipeOutput p_301014_, BlockFamily p_176582_) {
            ...
            (p_176529_, p_176530_) -> {
               ...
               if (bifunction != null) {
                  ...
                  p_176582_.getRecipeGroupPrefix()
                     .ifPresent(p_293701_ -> recipebuilder.group(p_293701_ + (p_176529_ == BlockFamily.Variant.CUT ? "" : "_" + p_176529_.getRecipeGroup())));
               ...
               if (p_176529_ == BlockFamily.Variant.CRACKED) {
                  smeltingResultFromBase(p_301014_, p_176530_, itemlike);
               }
            }
         );
   }
      ...
      return CriteriaTriggers.INVENTORY_CHANGED
         .createCriterion(
            new InventoryChangeTrigger.TriggerInstance(
               Optional.empty(), MinMaxBounds.Ints.ANY, MinMaxBounds.Ints.ANY, MinMaxBounds.Ints.ANY, List.of(p_126012_)
            )
         );
   }
