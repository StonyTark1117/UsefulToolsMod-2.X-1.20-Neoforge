...
public class ServerboundEditBookPacket implements Packet<ServerGamePacketListener> {
   ...
   public ServerboundEditBookPacket(FriendlyByteBuf p_179592_) {
      ...
      this.pages = p_179592_.readCollection(FriendlyByteBuf.limitValue(Lists::newArrayListWithCapacity, 200), p_182763_ -> p_182763_.readUtf(8192));
      this.title = p_179592_.readOptional(p_182757_ -> p_182757_.readUtf(128));
   }

   @Override
   public void write(FriendlyByteBuf p_134011_) {
      p_134011_.writeVarInt(this.slot);
      p_134011_.writeCollection(this.pages, (p_182759_, p_182760_) -> p_182759_.writeUtf(p_182760_, 8192));
      p_134011_.writeOptional(this.title, (p_182753_, p_182754_) -> p_182753_.writeUtf(p_182754_, 128));
   }
