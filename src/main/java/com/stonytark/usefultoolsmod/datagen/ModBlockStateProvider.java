...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    private void blockItem(DeferredHolder<Block, ? extends Block> blockDeferredHolder) {
        simpleBlockItem(blockDeferredHolder.get(), new ModelFile.UncheckedModelFile("tutorialmod:block/" +
                BuiltInRegistries.BLOCK.getKey(blockDeferredHolder.get()).getPath()));
    }

    private void blockItem(DeferredHolder<Block, ? extends Block> blockDeferredHolder, String appendix) {
        simpleBlockItem(blockDeferredHolder.get(), new ModelFile.UncheckedModelFile("tutorialmod:block/" +
                BuiltInRegistries.BLOCK.getKey(blockDeferredHolder.get()).getPath() + appendix));
    }
}